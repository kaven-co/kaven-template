...
const SO_STATUSES = ['DRAFT', 'CONFIRMED', 'PICKING', 'PACKED', 'SHIPPED', 'DELIVERED', 'CANCELLED'] as const;
...
export default function SalesOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['sales-orders', tenant?.id, statusFilter],
    queryFn: async () => {
      const params: Record<string, string> = {};
      if (statusFilter !== 'ALL') params.status = statusFilter;
      const res = await api.get('/api/v1/inventory/sales-orders', { params });
      return res.data;
    },
    enabled: !!tenant?.id,
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">Fulfillment pipeline</p>
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-40"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="ALL">All Status</SelectItem>
            {SO_STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-12 text-center">
          ...
          <h3 className="text-lg font-semibold">No sales orders</h3>
          <p className="text-muted-foreground">Sales orders will appear here when created</p>
        </Card>
      ) : (
        <div className="space-y-2">
          {orders.map((order: any) => (
            <Link key={order.id} href={`/inventory/sales-orders/${order.id}`}>
              <Card className="p-4 hover:border-primary/50 transition-colors cursor-pointer flex items-center justify-between">
                <div>
                  <h3 className="font-medium">{order.orderNumber}</h3>
                  <p className="text-sm text-muted-foreground">{order.customerName || 'No customer'} | {order._count?.items || 0} items</p>
                </div>
