...
export function useTenants(params?: { 
  ...
  const batchDeleteTenants = useMutation({
    mutationFn: async (ids: string[]) => {
      const response = await api.post('/api/tenants/batch-delete', { ids });
      return response.data as { deleted: number; ids: string[] };
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['tenants'] });
      queryClient.invalidateQueries({ queryKey: ['tenant-stats'] });
      toast.success(`${data.deleted} tenant(s) removido(s) com sucesso!`);
    },
    onError: (error: unknown) => {
      toast.error(getErrorMessage(error));
    },
