 ...
 * Retorna a zona IANA se encontrada na lista, senão retorna UTC
 */
export function detectUserTimezone(): string {
  try {
    const userZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    const exists = TIMEZONES.find(tz => tz.zone === userZone);
    return exists ? exists.zone : 'UTC';
  } catch {
    return 'UTC';
  }
}
...
/**
 * Encontra um timezone pela zona IANA
 */
export function findTimezoneByZone(zone: string): TimezoneOption | undefined {
