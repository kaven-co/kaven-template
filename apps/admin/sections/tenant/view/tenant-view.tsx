...
import { Button, ConfirmationModal } from '@kaven/ui-base';
...
export function TenantView() {
  ...
  // API Hooks
  const { tenants, pagination, isLoading, batchDeleteTenants } = useTenants({
  ...
  const handleConfirmBatchDelete = () => {
    batchDeleteTenants.mutate(selected, {
      onSuccess: () => {
        setSelected([]);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationModal
        ...
        message={t('batchDelete.message', { count: selected.length })}
        confirmLabel={t('batchDelete.confirm')}
        cancelLabel={tCommon('cancel')}
        variant="danger"
        isLoading={batchDeleteTenants.isPending}
      />
    </div>
  );
}
